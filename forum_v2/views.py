...
def add_comment(request, id):
    if request.method == "POST":
        try: 
            ...
            #     }
            # })
            return redirect('forum_v2:get_discussion_by_id_v2', id=id)
        except Exception as e:
...
def add_discussion(request):
    if request.method == "POST":
        ...
        #     }
        # })
            return redirect('forum_v2:discussion_v2')
...
def delete_detail_event(request, id):
    try:
        ...
        data.delete()
        return JsonResponse({'status': 'success', 'redirect_url': reverse('forum_v2:discussion_v2')})
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500) 
